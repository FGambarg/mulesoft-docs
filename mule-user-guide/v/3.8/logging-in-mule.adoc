...
If you want to log your Runtime Manager Agent state in a different location than the default 'mule_agent.log' file you can set up your log configuration file to do this.
...
This will make the Runtime Manager agent log its state to a rolling log file in '$MULE_HOME/logs/custom_mule_agent.log', which will roll on a per day basis and until the file reaches a 250MB size.
